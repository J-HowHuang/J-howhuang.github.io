...
find "$TARGET_DIR" -type f -name "*.md" | while read -r md_file; do
    ...
    while read -r line; do
        ...
        if [[ "$line" =~ \.(png|jpg|jpeg) ]]; then
            echo "  [LINE $line_num] Found potential image reference: '$line'"

            # Match standard markdown or HTML image syntax
            ...
            if [[ "$match" =~ \<img ]]; then
                # Strip everything before src= and remove surrounding quotes
                img_rel_path="${match#*src=}"
                img_rel_path="${img_rel_path%% *}" # trim trailing attributes if any
                img_rel_path="${img_rel_path//[\"\']/}" # strip quotes
                echo "    [SYNTAX] HTML <img> tag detected."
            else
                # Extract path from standard markdown ![alt](path)
            ...
            # -----------------------------------------------------------------
            # PHASE 1: Handle PNG Conversion
            # -----------------------------------------------------------------
            if [[ "$img_rel_path" =~ \.(png)$ ]]; then
                ...
                if [ -f "$img_abs_path" ]; then
                    ...
                    if convert "$img_abs_path" -background white -alpha remove -alpha off -quality 85 "$jpg_abs_path" 2>/dev/null; then
                        ...
                        echo "      [ERROR] ImageMagick 'convert' failed. Skipping resizing."
                        continue
                    fi
                else
            ...
            # -----------------------------------------------------------------
            # PHASE 2: Intelligent Resizing
            # -----------------------------------------------------------------
            if [ -f "$img_abs_path" ]; then
                echo "    [PROCESS] Checking dimensions for resizing: $img_abs_path"

                # Fetch dimensions with explicit error output
                dimensions=$(identify -format "%w %h" "$img_abs_path" 2>&1)
                ...
                if [[ "$dimensions" =~ ^[0-9]+\ [0-9]+$ ]]; then
                    ...
                    echo "      [IDENTIFY ERROR] ImageMagick identify output: $dimensions"
                fi
            else
                echo "    [NOT FOUND] Image target does not exist at: $img_abs_path"
            fi
