...
if [ ! -f "$TARGET_FILE" ]; then
    echo "[ERROR] File not found or is not a regular file: $TARGET_FILE"
    exit 1
fi

echo "================================================================="
echo "Working Directory (Base for Image Paths): $WORKING_DIR"
echo "Target Markdown File: $(cd "$(dirname "$TARGET_FILE")" && pwd)/$(basename "$TARGET_FILE")"
echo "================================================================="

echo -e "\n[DEBUG] Scanning single Markdown file for image references...\n"

echo "--------------------------------------------------------"
echo "[FILE] Processing markdown: $TARGET_FILE"
echo "--------------------------------------------------------"
            ...
            if [ -f "$img_abs_path" ]; then
                ...
                if convert "$img_abs_path" -background white -alpha remove -alpha off -quality 85 "$jpg_abs_path" 2>/dev/null; then
                    ...
                    if sed --version >/dev/null 2>&1; then
                        sed -i "s|$img_rel_path|$jpg_rel_path|g" "$TARGET_FILE"
                    else
                        sed -i "" "s|$img_rel_path|$jpg_rel_path|g" "$TARGET_FILE"
                    fi
